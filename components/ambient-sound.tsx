...
const normalizeVolume = (volume: number): number =>
  Math.max(0, Math.min(1, volume / MAX_VOLUME));
...
function AmbientSoundsWindow({
  ...
  const [sounds, setSounds] = React.useState<Sound[]>(() => {
    ...
    Object.entries(soundStructure).forEach(([category, names]) =>
      names.forEach((soundName) => {
        const extension =
          category === "binaural" || category === "noise" ? "wav" : "mp3";
        allSounds.push({
    ...
    if (saved) {
      ...
      allSounds.forEach((s) => {
        if (volMap[s.id]) s.volume = volMap[s.id];
      });
    }
    return allSounds;
  });
  ...
  const sortedMixes = [...savedMixes].sort(
      ...
      prev.map((s) => {
        if (s.id === id) {
          ...
          if (playing) {
            ...
            audio.volume =
              normalizeVolume(s.volume) * normalizeVolume(masterVolume);
  ...
  const handleVolumeChange = (id: string, val: number) => {
    ...
    if (audio)
      audio.volume = normalizeVolume(val) * normalizeVolume(masterVolume);
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <div className="p-4 rounded-xl border border-border/50 bg-background/80 flex-1 w-full">
              ...
              <input
                ...
                value={masterVolume}
                onChange={(e) =>
                  handleMasterVolumeChange(parseInt(e.target.value))
                }
                className="w-full"
              />
            </div>
